...
import { insertAttempt } from '../app/providers/supabase-write.js?v=2026-06-23-16-183713';
import { uniqueBaseCount, sampleKByBase, computeTargetTopics, interleaveBatches } from '../app/core/pick.js?v=2026-06-23-16-183713';
import {
  loadCatalogIndexLike,
  lookupQuestionsByIdsV1,
} from '../app/providers/catalog.js?v=2026-06-23-16-183713';
import { toAbsUrl } from '../app/core/url_path.js?v=2026-06-23-16-183713';

import { loadSmartMode, saveSmartMode, clearSmartMode, ensureSmartDefaults, isSmartModeActive } from './smart_mode.js?v=2026-06-23-16-183713';

import { questionStatsForTeacherV1 } from '../app/providers/homework.js?v=2026-06-23-16-183713';
import { pickProtosByPriority } from './pick_priority.js?v=2026-06-23-16-183713';
...
import { hydrateVideoLinks, wireVideoSolutionModal } from '../app/video_solutions.js?v=2026-06-23-16-183713';
import { safeEvalExpr } from '../app/core/safe_expr.mjs?v=2026-06-23-16-183713';
import { isPart2Question, renderPart2Stem, buildPart2EtalonBlock, buildPart2SelfScore, typesetEl } from './part2_render.js?v=2026-06-23-16-183713';
import { submitPart2SelfScore } from '../app/providers/part2.js?v=2026-06-23-16-183713';
import { setStem } from '../app/ui/safe_dom.js?v=2026-06-23-16-183713';
import { registerStandardPrintPageLifecycle } from '../app/ui/print_lifecycle.js?v=2026-06-23-16-183713';
import { getSession } from '../app/providers/supabase.js?v=2026-06-23-16-183713';
import { supaRest } from '../app/providers/supabase-rest.js?v=2026-06-23-16-183713';
import { confirmFinish } from '../app/ui/confirm_finish.js?v=2026-06-23-16-183713';
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Фильтры приоритезации с главной учителя (в тестировании работают только вне SMART)
  TEACHER_STUDENT_ID = String(sel.teacher_student_id || '').trim();
  TEACHER_PICKED_REFS = Array.isArray(sel.teacher_picked_refs)
    ? sel.teacher_picked_refs.map(normalizeSmartRef).filter(Boolean)
    : [];
  const tf = sel.teacher_filters || {};
  TEACHER_FILTERS = { old: !!tf.old, badAcc: !!tf.badAcc };
  PRIO_ACTIVE = !!TEACHER_STUDENT_ID && (TEACHER_FILTERS.old || TEACHER_FILTERS.badAcc) && !SMART_ACTIVE;
  if (!PRIO_ACTIVE) {
  ...
  try {
    ...
    if (SMART_ACTIVE) {
      ...
      if (TEACHER_PICKED_REFS.length) {
        perfMark('pickTeacherRefs:start');
        const direct = await buildQuestionsFromSmartRefs(TEACHER_PICKED_REFS);
        if (direct.length === TEACHER_PICKED_REFS.length) {
          questions = direct;
        }
        perfMark('pickTeacherRefs:done');
      }
...
async function bootSessionMode(token, overlay) {
  ...
  // WS.1-fix (2026-06-08): «Перемешать задачи» для session-ссылки. frozen_questions имеют
  // фиксированный порядок, а session-ветка минует pickPrototypes (где SHUFFLE_TASKS применялся) →
  // шафлим здесь по флагу. Без этого галочка «Перемешать» на главной не действовала (в ДЗ — работает,
  // там hw.js шафлит напрямую). SHUFFLE_TASKS уже выставлен из spec.shuffle выше.
...
async function ensureStatsMapForManifest(man) {
  try {
    ...
    const p = (async () => {
      ...
      if (!res?.ok) {
        console.warn('[prio] stats rpc failed (trainer)', res);
        return null;
      }
    ...
    console.warn('[prio] stats error (trainer)', e);
    return null;
  }
}
