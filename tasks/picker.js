...
import { withBuild } from '../app/build.js?v=2026-06-07-65';
import { supabase, getSession, signInWithGoogle, signOut, finalizeOAuthRedirect } from '../app/providers/supabase.js?v=2026-06-07-65';
import { CONFIG } from '../app/config.js?v=2026-06-07-65';
import { supaRest } from '../app/providers/supabase-rest.js?v=2026-06-07-65';
import { loadCatalogIndexLike } from '../app/providers/catalog.js?v=2026-06-07-65';
import { listMyStudents, questionStatsForTeacherV1, protoLast3ForTeacherV1, protoLast3ForSelfV1, loadTeacherPickingScreenV2, loadTeacherPickingResolveBatchV1 } from '../app/providers/homework.js?v=2026-06-07-65';
import { pickQuestionsScopedForList } from './pick_engine.js?v=2026-06-07-65';
import { setStem } from '../app/ui/safe_dom.js?v=2026-06-07-65';
import { toAbsUrl } from '../app/core/url_path.js?v=2026-06-07-65';
import { baseIdFromProtoId } from '../app/core/pick.js?v=2026-06-07-65';
import { createSessionLink } from '../app/providers/task_session.js?v=2026-06-07-65';
// W2.1' Variant B: pure resolve/manifest builders extracted to a self-contained module.
import { ensurePickerManifest, loadTopicPoolForPreview, normalizeResolveReqArray, buildResolveBucketKey, getResolveRowBucketKey } from './picker_added_tasks.js?v=2026-06-07-65';
...
import {
  ...
  BADGE_COLOR_CLASSES,
} from './picker_common.js?v=2026-06-07-65';
...
} from './picker_stats.js?v=2026-06-07-65';
...
function applyTeacherStudentView(studentId, opts = {}){
  ...
  if (!sid) {
    setHomeStatsLoading(false);
    // M1: ученик сброшен → блок «ПРОГНОЗ ЕГЭ» (desktop #scoreForecast + mobile #mForecast)
    // остаётся видимым, но без чисел (—) и со шкалой --sec=0. Иначе остаётся stale-балл
    // прошлого ученика. clearStudentLast10UI здесь не подходит: его guard isStudentLikeHome()
    // уже false (TEACHER_VIEW_STUDENT_ID сброшен в setTeacherStudentViewUI выше), поэтому
    // чистим прогноз напрямую (у updateScoreForecast внутреннего guard нет — caller-guard).
...
async function syncAddedTasksToSelection(opts = {}) {
  ...
  if (sectionNeedMap.size > 0) {
    ...
    if (sid) {
      ...
      // excludeQuestionIds. 12-секционный section-batch с complete упирается в statement_timeout
      // (HTTP 500 ~8.8 с) и тащит за собой 12-серийный фоллбэк (~18 с). Обходим его для uniform.
      // Покрывает и K=1 (бывшая canUseGlobalAll-ветка), и K≥2 (фикс).
      const deltaVals = Array.from(remaining.values()).map((d) => Math.max(0, Number(d || 0)));
      const uniformK = (
        deltaVals.length === (SECTIONS || []).length &&
        deltaVals.length > 0 &&
        deltaVals.every((d) => d === deltaVals[0])
      ) ? deltaVals[0] : 0;
      ...
      if (uniformK >= 1) {
        for (let round = 0; round < uniformK; round++) {
          ...
          const resolved = await pickQuestionsViaTeacherScreenResolve({
            ...
            excludeTopicIds: Array.from(excludeTopics),
            // свежий exclude КАЖДЫЙ раунд (append→incIdCount пополняет idCounts) → раунды дают РАЗНЫЕ задачи.
            excludeQuestionIds: getExcludeSet(),
          });
          if (seq !== _ADDED_SYNC_SEQ) return;
          ...
          for (const q of (Array.isArray(resolved) ? resolved : [])) {
            ...
            if (!sectionId || !sectionNeedMap.has(sectionId)) continue;
            appendPickedQuestionsToBucket(ctx, `section:${sectionId}`, [q]);
          }

          // WP2 (C): инкрементальный предпросмотр — открытая модалка наполняется по мере добора,
          // а не рывком в конце синка (убирает воспринимаемое «мигание» sum↔предпросмотр).
        ...
        // uniform добран K× global_all — section-batch (500-prone) для него НЕ нужен.
        remaining.clear();
      }
