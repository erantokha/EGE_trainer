...
import { uniqueBaseCount, sampleKByBase, computeTargetTopics, interleaveBatches } from '../app/core/pick.js?v=2026-02-17-5';

import { CONFIG } from '../app/config.js?v=2026-02-17-5';
import { getHomeworkByToken, startHomeworkAttempt, submitHomeworkAttempt, getHomeworkAttempt, normalizeStudentKey } from '../app/providers/homework.js?v=2026-02-17-5';
import { supabase, getSession } from '../app/providers/supabase.js?v=2026-02-17-5';
import { hydrateVideoLinks, wireVideoSolutionModal } from '../app/video_solutions.js?v=2026-02-17-5';


import { safeEvalExpr } from '../app/core/safe_expr.mjs?v=2026-02-17-5';
import { setStem } from '../app/ui/safe_dom.js?v=2026-02-17-5';
...
function renderReviewCards() {
  ...
  SESSION.questions.forEach((q, idx) => {
    ...
    // Mobile fix: place actions under correct answer to avoid overlap
    try{
      if (window.matchMedia && window.matchMedia('(max-width: 520px)').matches){
        const lines = ans.querySelectorAll('.hw-ans-line');
        const actions = ans.querySelector('.hw-actions');
        if (lines.length >= 2 && actions){
          // Move the actions block after "Правильный ответ"
          lines[1].after(actions);
        }
      }
    }catch(_e){}
