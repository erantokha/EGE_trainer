...
export default {
  async fetch(request) {
    ...
    // 9. OAuth Location: проксируем как есть.
    // Был эксперимент с rewrite redirect_uri (supabase.co → workers.dev), чтобы
    // увести Google-callback через прокси. Это ломает PKCE-flow: Supabase делает
    // server-side обмен code↔token у Google со своим каноническим redirect_uri,
    // а Google требует точного совпадения с тем, что был при /authorize. После
    // rewrite случаются mismatch и ошибка `Unable to exchange external code`.
    // Решение возможно только с Supabase Custom Auth Domain (Pro plan).
    // Сейчас: Google login = работает у тех, у кого supabase.co открыт (VPN);
    // email/password login = работает у всех через прокси.
