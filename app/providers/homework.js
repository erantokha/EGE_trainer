...
import { CONFIG } from '../config.js?v=2026-06-17-16-172628';
import { requireSession } from './supabase.js?v=2026-06-17-16-172628';
import { supaRest } from './supabase-rest.js?v=2026-06-17-16-172628';
...
// ===== Rollup по темам для учителя (v1) =====
// Возвращает { ok, rows, fn, error }.
// rows: массив строк (ожидаем {topic_id, total_questions, attempted_questions, never_questions, last_attempt_at_max, acc_avg, acc_min}).
export async function teacherTopicRollupV1({
  student_id,
  topic_ids,
  timeoutMs = 8000,
} = {}) {
...
// ===== Подбор задач по темам с квотами (учительские фильтры, v1) =====
// Возвращает { ok, rows, fn, error }.
// rows: массив строк (ожидаем {question_id, topic_id, type_id, section_id, manifest_path, ...}).
export async function pickQuestionsForTeacherTopicsV1({
  student_id,
  topics = null, // array[{id,n}] or map
  flags = null,
  exclude_ids = null,
  exclude_topic_ids = null,
  overfetch = 4,
