...
| question_stats_for_teacher_v1 | `question_stats_for_teacher_v1(p_student_id uuid, p_question_ids text[])` | TABLE(question_id text, total int, correct int, last_attempt_at timestamptz) | true | планируется использовать для приоритезации выбора задач на главной учителя (патч 2.2) |
