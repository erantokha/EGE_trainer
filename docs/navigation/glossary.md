...
- theme
  - верхний уровень учебного каталога; в legacy-коде часто соответствует бывшему `section`

- subtopic
  - каноническая подтема внутри `theme`; в legacy-коде часто соответствует бывшему `topic`

- topic
  - legacy-имя для `subtopic` в части старого фронтенда и старых SQL/JSON-структур

- unic
  - канонический уровень между `subtopic` и `question`; объединяет конкретные `question` одной сущности и соответствует former `type_id` / `unic_question_id` в переходных контурах

- question
  - конкретный экземпляр задачи внутри `unic`; обычно соответствует `proto.id` / `question_id`
