...
- `pick_questions_for_teacher_v1` → [pick_questions_for_teacher_v1.sql](pick_questions_for_teacher_v1.sql)
- `get_homework_by_token` → [get_homework_by_token.sql](get_homework_by_token.sql)
- `start_homework_attempt` → [start_homework_attempt.sql](start_homework_attempt.sql)
- `has_homework_attempt` → [has_homework_attempt.sql](has_homework_attempt.sql)
- `get_homework_attempt_by_token` → [get_homework_attempt_by_token.sql](get_homework_attempt_by_token.sql)
- `submit_homework_attempt` → [submit_homework_attempt.sql](submit_homework_attempt.sql)
- `get_homework_attempt_for_teacher` → [get_homework_attempt_for_teacher.sql](get_homework_attempt_for_teacher.sql)
- `assign_homework_to_student` → [assign_homework_to_student.sql](assign_homework_to_student.sql)
- `student_my_homeworks_summary` → [student_my_homeworks_summary.sql](student_my_homeworks_summary.sql)
- `student_my_homeworks_archive` → [student_my_homeworks_archive.sql](student_my_homeworks_archive.sql)
- `auth_email_exists` → [auth_email_exists.sql](auth_email_exists.sql)
- `update_my_profile` → [update_my_profile.sql](update_my_profile.sql)
- `delete_my_account` → [delete_my_account.sql](delete_my_account.sql)
- `add_student_by_email` → [add_student_by_email.sql](add_student_by_email.sql)
- `remove_student` → [remove_student.sql](remove_student.sql)
- `list_student_attempts` → [list_student_attempts.sql](list_student_attempts.sql)
...
| canonical_name | current_status | why_now | target_sql_file |
| --- | --- | --- | --- |
| `pick_questions_for_teacher_v1` | `standalone_sql` | legacy/compat picking-контур; live-SQL уже вынесен, пока живёт в runtime до удаления | `docs/supabase/pick_questions_for_teacher_v1.sql` |
...
| --- | --- | --- |
| `get_homework_by_token` | `standalone_sql` | `docs/supabase/get_homework_by_token.sql` |
| `start_homework_attempt` | `standalone_sql` | `docs/supabase/start_homework_attempt.sql` |
| `has_homework_attempt` | `standalone_sql` | `docs/supabase/has_homework_attempt.sql` |
| `get_homework_attempt_by_token` | `standalone_sql` | `docs/supabase/get_homework_attempt_by_token.sql` |
| `submit_homework_attempt` | `standalone_sql` | `docs/supabase/submit_homework_attempt.sql` |
| `get_homework_attempt_for_teacher` | `standalone_sql` | `docs/supabase/get_homework_attempt_for_teacher.sql` |
| `assign_homework_to_student` | `standalone_sql` | `docs/supabase/assign_homework_to_student.sql` |
| `student_my_homeworks_summary` | `standalone_sql` | `docs/supabase/student_my_homeworks_summary.sql` |
| `student_my_homeworks_archive` | `standalone_sql` | `docs/supabase/student_my_homeworks_archive.sql` |
...
| canonical_name | current_status | target_sql_file |
| --- | --- | --- |
| `auth_email_exists` | `standalone_sql` | `docs/supabase/auth_email_exists.sql` |
| `update_my_profile` | `standalone_sql` | `docs/supabase/update_my_profile.sql` |
| `delete_my_account` | `standalone_sql` | `docs/supabase/delete_my_account.sql` |
| `add_student_by_email` | `standalone_sql` | `docs/supabase/add_student_by_email.sql` |
| `remove_student` | `standalone_sql` | `docs/supabase/remove_student.sql` |
| `list_student_attempts` | `standalone_sql` | `docs/supabase/list_student_attempts.sql` |
...
- runtime SQL-gap этапа 0 закрыт; следующий практический фокус — оформить это как завершение этапа 0 и решить, нужен ли ещё `.github/CODEOWNERS` перед переходом к этапу 1
