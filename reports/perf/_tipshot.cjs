...
await p.waitForSelector('#htSidebar .ht-sidebar-item',{timeout:20000}).catch(()=>{});await s(1500);
await p.locator('#htNavStats').hover();await s(400);
// геометрия ::after через range? нет. Возьмём bounding box элемента + проверим overflow предков
const diag=await p.evaluate(()=>{
  const it=document.querySelector('#htNavStats');const r=it.getBoundingClientRect();
  let el=it,chain=[];
  while(el && el!==document.body){const cs=getComputedStyle(el);if(cs.overflow!=='visible'||cs.overflowX!=='visible'||cs.overflowY!=='visible')chain.push(el.className+' {'+cs.overflow+'/'+cs.overflowX+'/'+cs.overflowY+'}');el=el.parentElement;}
  const sb=document.querySelector('#htSidebar');const sp=document.querySelector('.ht-sidebar-panel');
  return {itemRect:{x:Math.round(r.x),y:Math.round(r.y),w:Math.round(r.width)}, clipAncestors:chain, sidebarCls:sb&&sb.className, panelOverflow:sp?getComputedStyle(sp).overflow:null, navOverflow:getComputedStyle(document.querySelector('.ht-sidebar-nav')).overflow};
});
console.log(JSON.stringify(diag,null,1));
await p.screenshot({path:'reports/perf/_tipshot.png',clip:{x:0,y:0,width:400,height:520}});
