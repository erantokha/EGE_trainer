...
async function shot(p,path){const acc=await p.$('#accordion');if(acc)await acc.screenshot({path});else await p.screenshot({path});}
...
await p.waitForSelector('#accordion .node.section',{timeout:20000}).catch(()=>{});await s(3500);
await shot(p,'reports/perf/_acc_student.png');
...
await p.waitForFunction(()=>{const s=document.getElementById('teacherStudentSelect');return s&&[...s.options].some(o=>o.value);},{timeout:25000}).catch(()=>{});await s(1000);
...
await p.waitForSelector('#accordion .node.section',{timeout:20000}).catch(()=>{});await s(4500);
await shot(p,'reports/perf/_acc_teacher.png');
