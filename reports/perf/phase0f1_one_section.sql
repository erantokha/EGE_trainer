...
with params as (
  select
    ...
    '{}'::jsonb as selection_json, (select jsonb_build_array(jsonb_build_object('scope_kind','section','scope_id', theme_id, 'n', 7)) from public.catalog_theme_dim where coalesce(is_enabled,true) and not coalesce(is_hidden,false) order by sort_order limit 1) as requests_json,
