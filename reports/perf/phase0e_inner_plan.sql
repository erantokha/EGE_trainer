-- ФАЗА 0e — ВНУТРЕННИЙ план resolve напрямую (без обёртки-функции → видны тайминги по CTE-узлам).
-- READ-ONLY. Запусти E1 (1 секция) и E2 (12 секций), пришли ВЕСЬ вывод (важны узлы с большим "actual time").
-- Это покажет, какой именно CTE/узел съедает ~0.55с (1 секция) и как растёт на 12.

-- ===== E1: ОДНА секция (n=7, weak_spots, complete) =====
EXPLAIN (ANALYZE, BUFFERS)
with params as (
  select
    'f1d03f75-08ad-48e6-9128-8f69afefe81e'::uuid as student_id,
    'all'::text as source,
    'weak_spots'::text as filter_id,
    'Слабые места'::text as filter_label,
    '{}'::jsonb as selection_json,
    (select jsonb_build_array(jsonb_build_object('scope_kind','section','scope_id', theme_id, 'n', 7))
       from public.catalog_theme_dim where coalesce(is_enabled,true) and not coalesce(is_hidden,false) order by sort_order limit 1) as requests_json,
    'fixed-seed-eee'::text as session_seed,
    '{}'::text[] as exclude_question_ids,
    true as complete
  ),
...
-- ===== E2: 12 секций (n=7 каждой, weak_spots, complete) =====
EXPLAIN (ANALYZE, BUFFERS)
with params as (
  select
    'f1d03f75-08ad-48e6-9128-8f69afefe81e'::uuid as student_id,
    ...
    'Слабые места'::text as filter_label,
    '{}'::jsonb as selection_json,
    (select jsonb_agg(jsonb_build_object('scope_kind','section','scope_id', theme_id, 'n', 7))
       from public.catalog_theme_dim where coalesce(is_enabled,true) and not coalesce(is_hidden,false)) as requests_json,
    'fixed-seed-eee'::text as session_seed,
    '{}'::text[] as exclude_question_ids,
    true as complete
  ),
