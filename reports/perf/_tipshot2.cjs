...
// проверим, что ::after реально в видимой зоне: bounding через getClientRects недоступен для ::after,
// но overflow цепочки уже visible — снимем картинку
await p.screenshot({path:'reports/perf/_tipshot2.png',clip:{x:0,y:0,width:400,height:340}});
const chain=await p.evaluate(()=>{let el=document.querySelector('#htNavStats'),c=[];while(el&&el!==document.body){const cs=getComputedStyle(el);if(cs.overflow!=='visible')c.push(el.className+':'+cs.overflow);el=el.parentElement;}return c;});
console.log('клиппящие предки (ждём пусто):',JSON.stringify(chain));
