...
const r=await p.evaluate(()=>{const b=document.getElementById('htSidebarUserBtn').getBoundingClientRect();return{x:Math.round(b.x),y:Math.round(b.y),w:Math.round(b.width),h:Math.round(b.height)};});
console.log('user-btn rect:',JSON.stringify(r));
await p.locator('#htSidebarUserBtn').hover();await s(400);
const cy=Math.max(0,r.y-30);
await p.screenshot({path:'reports/perf/_av_collapsed.png',clip:{x:0,y:cy,width:300,height:Math.min(120,900-cy)}});
await p.evaluate(()=>document.querySelector('#htSidebar')?.classList.add('open'));await s(500);
await p.screenshot({path:'reports/perf/_av_open.png',clip:{x:0,y:cy,width:300,height:Math.min(120,900-cy)}});
