-- ВРЕМЕННАЯ функция (S2/S3 инлайн + MATERIALIZED на candidate_base/selected_proto_rows/question_candidates).
-- Цель: убрать пере-выполнение ранжирующего Append 3561× (root cause из phase0f). Семантика идентична.
-- Перезапишет teacher_picking_resolve_batch_perf_v1. После: DROP FUNCTION ... perf_v1(...).

-- ВРЕМЕННАЯ функция для A/B (S2/S3: инлайн-состояние, один скан, без last3/двойного вызова).
-- Копия teacher_picking_resolve_batch_v1; live-функцию НЕ трогает. Семантика идентична (см. PARITY).
-- После замеров:  DROP FUNCTION IF EXISTS public.teacher_picking_resolve_batch_perf_v1(uuid,text,text,jsonb,jsonb,text,text[],boolean);

-- teacher_picking_resolve_batch_perf_v1.sql
...
create or replace function public.teacher_picking_resolve_batch_perf_v1(
  p_student_id uuid,
...
begin
  ...
  -- Доступ: self (ученик о себе — для быстрого батч-подбора с фильтром на home_student) ИЛИ
  -- teacher своего ученика. v_uid := auth.uid() (выше). Зеркало self-гейта teacher_picking_screen_v2.
  -- Self видит только свои данные (student_topic_state_v1 по p_student_id); anon (v_uid=NULL) отсечён выше.
...
revoke execute on function public.teacher_picking_resolve_batch_perf_v1(
  uuid, text, text, jsonb, jsonb, text, text[], boolean
) from anon;

grant execute on function public.teacher_picking_resolve_batch_perf_v1(
  uuid, text, text, jsonb, jsonb, text, text[], boolean
) to authenticated;
