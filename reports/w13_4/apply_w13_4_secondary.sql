...
begin
  ...
  raw_events as (
    ...
    -- W13.4: часть 2 (№13) не оценивается из answer_events (там correct=false), её балл живёт
    -- в part2_attempt_reviews и уже отражён в градуснике (subtopic_last3_avg_pct из topic_state).
    -- Исключаем №13 из оконных метрик/overall, чтобы не показывать ложные 0% и не занижать общую точность.
    and ecm.theme_id <> '13'
  ),
  ...
  topic_rows_data as (
    select
      ...
      -- WSA-1b: уже посчитанные в layer-3 прото-агрегаты (аддитивная проекция, без новой логики).
      coalesce(ts.weak_proto_count, 0)::int                as weak_proto_count,
      coalesce(ts.unstable_proto_count, 0)::int            as unstable_proto_count,
      coalesce(ts.stale_proto_count, 0)::int               as stale_proto_count,
      coalesce(ts.solved_proto_count, 0)::int              as solved_proto_count,
      coalesce(ts.independent_correct_proto_count, 0)::int as mastered_proto_count,
      ts.subtopic_last3_avg_pct,
    ...
    select coalesce(
      jsonb_agg(
        jsonb_build_object(
          ...
          'weak_proto_count', t.weak_proto_count,
          'unstable_proto_count', t.unstable_proto_count,
          'stale_proto_count', t.stale_proto_count,
          'solved_proto_count', t.solved_proto_count,
          'mastered_proto_count', t.mastered_proto_count,
          'accuracy', t.accuracy,
          'attempt_count_total', t.attempt_count_total,
          'correct_count_total', t.correct_count_total,
          'coverage', jsonb_build_object(
